 ...
 */
const node = 'node';
const program = 'tibcli-wi';

describe('tibcli-wi.js', function () {
    this.slow(10000);
    it('should be version ' + mod.version, () => {
        var result = spawnSync('node', [program, '--version'], { encoding: 'utf-8' });
        expect(result.stdout).to.include(mod.version);
    });
    it('should contain the correct functions', () => {
        var result = spawnSync('node', [program, '--help'], { encoding: 'utf-8' });
        expect(result.stdout).to.include(config.commands.sdk.description);
        expect(result.stdout).to.include(config.commands.init.description);
        expect(result.stdout).to.include(config.commands.add.description);
